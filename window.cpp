...
namespace spritelib
{
	...
	void Window::set_ortho_window(float a_x, float a_y, float a_width, float a_height)
	{
		m_proj = new math::Matrix4x4();
		*m_proj = math::Matrix4x4::ortho(a_x, a_x + a_width, a_y, a_y + a_height, -1.0f, 1.0f);
	}

	//this is to zoom in or out of screen
	Window & Window::set_ortho_matrix(float a_x, float a_y, float a_width, float a_height)
	{
		delete m_proj;
		m_proj = new math::Matrix4x4(
			math::Matrix4x4::ortho(
				a_x, a_x + a_width, a_y, a_y + a_height, -100.0f, 100.0f).m_data);
